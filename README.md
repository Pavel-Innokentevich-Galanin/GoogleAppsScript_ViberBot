...
- создал команду `/sendPhone`, по которой бот отправит клавиатуру для отправки телефона.
    - Телефон запишется в таблицу `СПР_Пользователи`
- создал команду `/setName Джон`, по которой бот будет знать имя
    - Когда пользователь запускает бота, то сохраняется Viber-имя в таблицу `СПР_Пользователи`
    ...
    - По команде `/setName Боб` в таблицу `СПР_Пользователи` запишется имя Боб
