...
- создал команду `/sendPhone`, по которой бот отправит клавиатуру для отправки телефона.
    - Телефон запишется в Google Таблицу на лист `СПР_Пользователи`
- создал команду `/setName Джон`, по которой бот будет знать имя
    - Когда пользователь запускает бота, то сохраняется Viber-имя в Google Таблицу на лист `СПР_Пользователи`
    - Если в настройках пользователь запретил выдачу имени, то будет имя 'Subscriber'
    - `Настройки` > `Конфиденциальность` > `Личные данные` > `Персонализация контента`
    - Чтобы не просить включать настройки, есть команда `/setName Чарли`, которая ставит любое имя
    - По команде `/setName Боб` в Google Таблицу на лист `СПР_Пользователи` запишется имя Боб
- добавил прослушку event `message`
    - Сообщение пользователя, отправленная картинка сохраняются в Google таблицу на лист `event_message`
- добавил прослушку event `conversation_started`
    - В Google Таблицу на лист `event_conversation_started` приходит сообщение, что пользователь присоединился к боту
- добавил прослушку event `seen`
    - В Google Таблицу на лист `event_seen` приходит сообщение, что пользователь прочел сообщение
- добавил прослушку event `delivered`
    - В Google Таблицу на лист `event_delivered` приходит сообщение, что пользователю дошло сообщение
- добавил прослушку event `unsubscribed`
    - В Google Таблицу на лист `event_unsubscribed` приходит сообщение, что пользователь отписался от бота
- добавил прослушку event `webhook`
    - В Google Таблицу на лист `event_webhook` приходит сообщение, что кто-то поменял ссылку боту
- добавил прослушку event `failed`
    - В Google Таблицу на лист `event_failed`
